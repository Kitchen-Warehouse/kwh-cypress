...
describe("Product Listing Page Filter Button Tests", () => {
  ...
  it("should show filtered state after Brand selection", () => {
    ...
    cy.get('[class*="ProductFilters_filterDesktopContainer"]').within(() => {
      cy.contains("Appetito").parent().find("input").check();
    });

    // Verify filtered UI appears
    cy.get('[class*="product-liststickySelectedFilters"]').should(
      "contain",
      "Appetito"
    );
  });
...
it('should show filtered state after Brand selection', () => {
  ...
  cy.get(".xl\\:flex .filters-btn-text").click();
  cy.get('[class*="ProductFilters_filterDesktopContainer"]').within(() => {
    cy.contains('Appetito').parent().find('input[type="checkbox"]').check({ force: true });
  });

  // Verify tag visible
  cy.contains('button', 'Appetito').should('be.visible');
  cy.get('.ProductCardproductCard, [class*="productCard"]').should('have.length.gte', 1);
});
});
